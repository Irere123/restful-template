	...
	const defaultProps = {
		children: (
			<>
				...
				<HugeiconsIcon icon={UnfoldMoreIcon} className={selectTriggerIconClassName} />
			</>
		),
	...
	return (
		<SelectPrimitive.Portal {...portalProps}>
			<SelectPrimitive.Positioner
				...
				anchor={anchor}
				className="z-50 max-w-(--available-width) select-none"
			...
			>
				...
				>
					...
					<div className="relative h-full min-w-(--anchor-width) overflow-hidden rounded-lg border bg-popover not-dark:bg-clip-padding shadow-lg/5 before:pointer-events-none before:absolute before:inset-0 before:rounded-[calc(var(--radius-lg)-1px)] before:shadow-[0_1px_--theme(--color-black/4%)] dark:before:shadow-[0_-1px_--theme(--color-white/6%)]">
...
export function SelectItem({
	...
	return (
		<SelectPrimitive.Item
			className={cn(
				"grid min-h-8 in-data-[side=none]:min-w-[calc(var(--anchor-width)+1.25rem)] cursor-default grid-cols-[1rem_1fr] items-center gap-2 rounded-md py-1.5 ps-2.5 pe-4 text-base outline-none data-disabled:pointer-events-none data-highlighted:bg-accent data-highlighted:text-accent-foreground data-disabled:opacity-64 sm:min-h-7.5 sm:text-sm [&_svg:not([class*='size-'])]:size-4.5 sm:[&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none [&_svg]:shrink-0",
				className,
			)}
